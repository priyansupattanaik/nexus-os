  ...
  return (
    <div className="flex flex-col h-full p-4 gap-4 bg-black/20">
      ...
      <div className="text-center py-2 border-b border-white/5">
        ...
        <div
          className={`text-3xl font-bold tracking-tight ${balance >= 0 ? "text-emerald-400" : "text-rose-400"}`}
        >
          {currency}
          {balance.toFixed(2)}
        </div>
      ...
      <div className="flex flex-col gap-2">
        ...
        <div className="flex gap-2">
          <button
            ...
            className="flex-1 os-btn bg-emerald-500/10 border-emerald-500/20 text-emerald-300 hover:bg-emerald-500/20"
          >
            <TrendingUp className="w-3 h-3" /> Credit
          </button>
          <button
            onClick={() => handleAdd("expense")}
            className="flex-1 os-btn bg-rose-500/10 border-rose-500/20 text-rose-300 hover:bg-rose-500/20"
          >
      ...
      <div className="flex-1 overflow-y-auto pr-1 space-y-1 custom-scrollbar">
        {txns.map((t) => (
          <div
            key={t.id}
            className="flex justify-between items-center py-2 px-3 rounded-lg hover:bg-white/5 transition-all group"
          >
            ...
            <div className="flex items-center gap-3">
              <span
                className={`font-mono text-xs font-semibold ${t.type === "income" ? "text-emerald-400" : "text-rose-400"}`}
              >
                {t.type === "income" ? "+" : "-"}
                {currency}
                {t.amount}
