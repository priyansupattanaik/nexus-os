...
def ask_nexus_ai(prompt: str, context: dict) -> str:
    if not client: return "AI Module Offline: Please set GROQ_API_KEY on the server."
    
    system = f"You are NEXUS, an OS AI. [USER DATA]: {str(context)}. Be technical and concise."
...
def process_command(req: AIRequest, db = Depends(get_authenticated_db)):
    # Pull fresh context from the database for THIS user
...
def get_briefing(db = Depends(get_authenticated_db)):
    ...
    return {"message": ask_nexus_ai("Generate a system status report.", {"tasks": tasks})}